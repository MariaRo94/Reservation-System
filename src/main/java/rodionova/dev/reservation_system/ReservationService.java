...
public class ReservationService {
    ...
    public Reservation updateReservation(Reservation reservationToUpdate) {
        if (!reservationMap.containsKey(reservationToUpdate.id())) {
            throw new IllegalArgumentException("No found reservation by id= " + reservationToUpdate.id());
        }
        var reservation = reservationMap.get(reservationToUpdate.id());
        ...
        var updatedReservation = new Reservation(
                ...
                ReservationStatus.PENDING);
        reservationMap.put(reservationToUpdate.id(), updatedReservation);
        return updatedReservation;
    }

    public void deleteReservation(Long id) {
        if (!reservationMap.containsKey(id)) {
            throw new IllegalArgumentException("No found reservation by id= " + id);
        }
    ...
    private boolean isReservationConflict(Reservation reservation) {
        for (Reservation existingReservation : reservationMap.values()){
            ...
            if(!existingReservation.roomId().equals(reservation.id())){
                continue;
            }
